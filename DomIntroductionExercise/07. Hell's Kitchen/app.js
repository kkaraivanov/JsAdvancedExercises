function solve() {
   ...
   const bestName = input => Object.entries(input).sort((x, y) => average(y[1]) - average(x[1]))[0];
   const average = workers => workers.reduce((x, y) => x + y[1], 0) / workers.length
   
   function inputParse(input) {
      const getWorkers = input => input.split(", ").map(x => x.split(" ").map(y => (isNaN(y) ? y : Number(y))))
      return JSON.parse(input).map(x => x.split(" - ")).reduce((x, y) => {
              const [name, workers] = y;
              x[name] = x[name] ? [...x[name], ...getWorkers(workers)] : getWorkers(workers)
              return x;
          }, {})
  }
